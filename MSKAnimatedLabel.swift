...
public class MSKAnimatedLabel: SKNode {
    ...
    public init(text: String, horizontalAlignment: SKLabelHorizontalAlignmentMode = .center, durationPerCharacter: Double = 0.05, fontSize: CGFloat = 40, marginVertical: CGFloat = 15.0, fontColor: SKColor = .black, fontName: String = "SalsaScripture20-Regular.otf", skipSpaces: Bool = true, labelWidth: CGFloat = 0.0, finishTypingOnTouch: Bool = false) {
    ...
    private func createLabels(forceNoAnimation: Bool = false) {
        ...
        lines?.forEach { line in
            let label = SKLabelNode(fontNamed: "SalsaScripture20-Regular")
    ...
    private func addTouchNodeForAutocomplete() {
        let touchNode = SKSpriteNode(color: .clear,
                                     size: .init(width: labelWidth, height: CGFloat(200)))
